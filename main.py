...
def main():
    ...
    co.set_argument('--blink-settings=imagesEnabled=false') # 禁用图片加快加载
    ...
    try:
        ...
        # 2. 读取 GitHub Secrets 注入的 Clerk 核心凭证 (新增了 ACTIVE_CONTEXT)
        ...
        if client_val and client_uat_val and active_context:
            ...
            cookies_to_set = [
                ...
                {'name': 'clerk_active_context', 'value': active_context, 'domain': '.pella.app'}
            ]
        ...
        if packet and packet.request.method == 'POST':
            ...
            # 新增调试功能：打印页面源码，排查是否被 Cloudflare 盾拦截
            print("\n--- 页面 HTML 快照前 1000 字符 (用于排错) ---")
